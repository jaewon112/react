...
function IndexMain({todoList, setTodoList}) {
    ...
    const [ contentOpen, setContentOpenId ] = useState(0);
    ...
    const handleOnKeyDown = e => {
        ...
        if (inputValue.trim().length === 0) { // trim() 양쪽 공백 제거
            setInputValue("")
            return;
        }
        console.log("todo 등록");
                ...
                <ul>
                    {
                        todoList.map(todo => (
                            <li key={todo.id}>
                                ...
                                <div css={s.todoTextContainer(contentOpen === todo.id)} onClick={() => handleContentOpenOnClick(todo.id)}>{todo.content}</div>
